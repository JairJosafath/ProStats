...
const LPlayerStats = () => {
  ...
  const handleSave = (playerstatsTracker) => {
    // console.log("tryna f", playerstatsTracker);
    ...
    if (
      ...
      const playerTable = tournament?.playerTable?.items?.filter(
        (table) => table.player.id === currentPlayer.id
      )[0];
      const oldStats = currentFixture?.playerStats?.items?.filter(
        (stats) => stats.player.id === currentPlayer?.id
      )[0];
      ...
      if (
        ...
        playerTable.skillmove_beat -= oldStats.possession_skillmove_beat;
        ...
        playerTable.expected_assists -= oldStats.passing_expected_assists;
        playerTable.interceptions -= oldStats.defending_interceptions;
        playerTable.tackles_won -= oldStats.defending_interceptions;
        ...
        setUpdatePlayerTableStat({
          ...
          goals: playerstatsTracker.summary_goals
            ? playerstatsTracker.summary_goals + playerTable.goals
            : 0 + playerTable.goals,
          assists: playerstatsTracker.summary_assists
            ? playerstatsTracker.summary_assists + playerTable.assists
            : 0 + playerTable.assists,
          beat: playerstatsTracker.possession_beat
            ? playerstatsTracker.possession_beat + playerTable.beat
            : 0 + playerTable.beat,
          skillmove_beat: playerstatsTracker.possession_skillmove_beat
          ...
          nutmeg: playerstatsTracker.possession_nutmeg
            ? playerstatsTracker.possession_nutmeg + playerTable.nutmeg
            : 0 + playerTable.nutmeg,
          match_rating: 0,
          expected_assists: playerstatsTracker.passing_expected_assists
            ? playerstatsTracker.passing_expected_assists +
              playerTable.expected_assists
            : 0 + playerTable.expected_assists,
          interceptions: playerstatsTracker.defending_interceptions
            ? playerstatsTracker.defending_interceptions +
              playerTable.interceptions
            : 0 + playerTable.interceptions,
          tackles_won: playerstatsTracker.defending_tackles_won
            ? playerstatsTracker.defending_tackles_won + playerTable.tackles_won
            : 0 + playerTable.tackles_won,
          blocks: playerstatsTracker.defending_blocks
            ? playerstatsTracker.defending_blocks + playerTable.blocks
            : 0 + playerTable.blocks,
          saves: playerstatsTracker.goalkeeping_saves
            ? playerstatsTracker.goalkeeping_saves + playerTable.saves
            : 0 + playerTable.saves,
          playerOfTheMatch: 0,
        });
      } else {
      ...
      setCreatePlayerStats({
        ...
        moderators: [
          ...
          ...(league.moderatornames ? league.moderatornames : []),
        ],
